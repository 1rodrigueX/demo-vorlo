...
import { createErpProposta, type ActionState } from "@/lib/actions/erp-propostas";
...
export function NovaPropostaForm({
  ...
  function handleSubmit() {
    ...
    fd.set("freightType", terms.freightType);
    fd.set("carrierId", terms.carrierId);
    fd.set("freightReais", String(freight));
    fd.set("discountReais", String(discount));
    fd.set("notes", notes);
    fd.set(
      "items",
      JSON.stringify(
      ...
      <Modal open={pdfPreviewOpen} onClose={() => setPdfPreviewOpen(false)} title="Pré-visualização da proposta" className="max-w-3xl max-h-[85vh] overflow-y-auto">
        ...
        <DocumentLayout
          ...
          meta={[
            ...
            { label: "Transportadora", value: carrierName || "—" },
          ]}
          footer={
        ...
        >
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.filter((i) => i.produtoId).map((item) => (
