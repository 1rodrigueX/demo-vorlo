...
import { createErpTenant, type ActionState } from "@/lib/actions/dev-erp";
import { Button } from "@/components/ui/Button";
import { Input } from "@/components/ui/Input";
import { PasswordInput } from "@/components/ui/PasswordInput";
import { Label } from "@/components/ui/Label";

export function ErpTenantForm({ onSaved }: { onSaved?: () => void }) {
  const [state, formAction, isPending] = useActionState<ActionState, FormData>(createErpTenant, null);
  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-4">
      <div>
        ...
        <Input id="e-name" name="name" required placeholder="Ex: Minha Empresa LTDA" />
      </div>
      <div>
        <Label htmlFor="e-slug">Slug</Label>
        <Input id="e-slug" name="slug" required placeholder="ex: minha-empresa" />
      </div>
