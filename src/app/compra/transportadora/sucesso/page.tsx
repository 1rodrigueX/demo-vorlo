...
import { checkTransportadoraProvisioningStatus } from "@/lib/actions/transportadora-checkout";

const POLL_INTERVAL_MS = 2000;
const POLL_TIMEOUT_MS = 20000;

export default function TransportadoraCheckoutSuccessPage() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      const { ready } = await checkTransportadoraProvisioningStatus();
      if (cancelled) return;
        ...
        {timedOut ? (
          ...
          <>
            <p className="mt-2 flex items-center justify-center gap-2 text-sm text-gray-600">
              ...
              Liberando seu acesso, só um instante...
            </p>
            <p className="mt-1 text-xs text-gray-400">
              Você vai ser levado pra página de download assim que estiver pronto.
            </p>
          </>
        )}
      </div>
