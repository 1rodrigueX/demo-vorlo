-- Da acesso ao FALA AI pra consultar (no banco, nao ao vivo) se uma
-- integracao (Bling, Anthropic, Gmail, Outlook) esta realmente conectada,
-- em vez de so orientar como conectar. Precisa atualizar o trigger (tenants
-- novos) e fazer backfill dos tenants existentes, igual 0031 fez pro prompt.
...
-- Backfill: FALA AI dos tenants existentes ganha a ferramenta e a regra nova.
...
set
  system_prompt =
    ...
    '- Confirme sempre o nome do agente e a ação executada (criado, atualizado, ativado, desativado, excluído) na resposta final.' || chr(10) ||
    '- Seja direto e objetivo. Responda sempre em português do Brasil.',
  tools = array['search_contacts', 'list_open_deals', 'set_deal_budget', 'mark_proposal_sent', 'mark_deal_won',
                'register_contact_in_bling', 'remember_fact', 'create_agent', 'update_agent', 'list_agents',
                'toggle_agent_status', 'delete_agent', 'connect_integration', 'check_integration_status']
