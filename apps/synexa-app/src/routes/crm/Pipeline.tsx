            ...
            return (
              <div key={s.id} className="flex w-64 shrink-0 flex-col rounded-2xl border border-carbon-800 bg-carbon-950/50">
                ...
                <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-2.5">
                  {cards.length === 0 ? (
                    <p className="px-1 py-6 text-center text-xs text-grey-dim">Vazio</p>
                  ) : (
                    cards.map((d) => (
                      <div key={d.id} className="rounded-xl border border-carbon-800 bg-carbon-800 p-3 transition-colors hover:border-carbon-600">
